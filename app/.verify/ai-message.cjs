...
// util/ai-message.js
var ai_message_exports = {};
__export(ai_message_exports, {
  buildAssistantBlocks: () => buildAssistantBlocks,
  buildPreviewFileUrl: () => buildPreviewFileUrl,
  buildUsageSummary: () => buildUsageSummary,
  formatDuration: () => formatDuration,
  formatScore: () => formatScore,
  normalizeMessageFiles: () => normalizeMessageFiles,
  normalizeRetrieverResources: () => normalizeRetrieverResources
});
module.exports = __toCommonJS(ai_message_exports);
...
var defaultLinkOpen = markdown.renderer.rules.link_open || function linkOpen(tokens, idx, options, env, self) {
...
var chartFenceLanguages = /* @__PURE__ */ new Set(["echarts", "chart"]);
...
function isEchartsOption(value) {
  ...
  return !!(value.series || value.xAxis || value.yAxis || value.radar || value.angleAxis || value.geo || value.calendar || value.parallel);
}
...
function hashText(text2 = "") {
  ...
  for (let index = 0; index < text2.length; index += 1) {
    hash = (hash << 5) - hash + text2.charCodeAt(index);
    hash |= 0;
  }
  return Math.abs(hash).toString(16);
}
function withStableBlockIds(blocks = []) {
  return blocks.map((block2, index) => {
    const seed = [
      block2.type,
      block2.title,
      block2.raw,
      block2.description,
      JSON.stringify(block2.option || {})
    ].filter(Boolean).join("|");
    return {
      ...block2,
      id: `${block2.type || "block"}-${index}-${hashText(seed)}`
    };
  });
}
function parseChartBlock(language, code2) {
  const lang = (language || "").trim().toLowerCase();
  const payload = tryParseJson((code2 || "").trim());
  if (!payload) {
    return null;
  }
  const option = isObject2(payload.option) ? payload.option : isEchartsOption(payload) ? payload : null;
  if (!option) {
    return null;
  }
  ...
  const title = isObject2(payload.title) ? payload.title.text || "" : payload.title || option.title?.text || "";
...
function pushMarkdownBlock(blocks, text2) {
  const html = renderMarkdown(text2);
  if (!html) {
    return;
  }
  blocks.push(
...
function splitMarkdownAndCharts(text2) {
  const blocks = [];
  const source = text2 || "";
  const fencePattern = /```([a-zA-Z0-9_-]+)?\s*\r?\n([\s\S]*?)```/g;
  let cursor = 0;
  let match2 = null;
  while ((match2 = fencePattern.exec(source)) !== null) {
    const [fullMatch, language, code2] = match2;
    const before = source.slice(cursor, match2.index);
    pushMarkdownBlock(blocks, before);
    const chartBlock = parseChartBlock(language, code2);
    if (chartBlock) {
      blocks.push(chartBlock);
    } else {
      pushMarkdownBlock(blocks, fullMatch);
    }
    cursor = match2.index + fullMatch.length;
  }
...
function normalizeAgentThoughts(agentThoughts = []) {
  ...
  return agentThoughts.map((item, index) => {
    const lines = [
      item?.thought,
      item?.message,
      item?.observation,
    ...
    const title = item?.tool || item?.tool_name || item?.node_name || item?.label || `\u601D\u8003 ${index + 1}`;
...
function splitThinkBlocks(content) {
  ...
  let cursor = 0;
  let match2 = null;
  while ((match2 = thinkPattern.exec(source)) !== null) {
    const before = source.slice(cursor, match2.index);
    blocks.push(...splitMarkdownAndCharts(before));
    const thinkText = (match2[1] || "").trim();
    if (thinkText) {
      blocks.push(
        createBlock("think", {
          title: "\u601D\u8003\u8FC7\u7A0B",
...
function buildAssistantBlocks(content = "", agentThoughts = []) {
...
function normalizeRetrieverResources(resources = []) {
...
function formatDuration(seconds) {
  ...
  if (value < 1) {
    return `${Math.round(value * 1e3)}ms`;
  }
...
function buildPreviewFileUrl(resolveUrl, appId, fileId, asAttachment = false) {
