<template>
	<div class="flow-progress" :style="{ height: `${getContainerHeight()}px` }">
		...
		<template v-for="(step, idx) in steps" :key="step.key">
			<div class="flow-node" :class="getNodeClass(step, idx)" :style="getNodeStyle(idx)">
				<div class="node-content" @click="showNodeDetail(step)">
					<span class="node-title">{{ step.Name }}</span>
					<!-- <span class="node-percent">({{ step.percent }}%)</span> -->
					<u-icon v-if="step.Children.length>0" color="rgb(0 0 0)" size="12" name="arrow-down"></u-icon>
				</div>
			</div>

			<!-- 箭头，最后一个节点不显示 -->
			<template v-if="idx < steps.length - 1">
				<div class="flow-arrow" :style="getArrowStyle(idx)" :class="getArrowClass(idx)">
					<svg viewBox="0 0 100 100" preserveAspectRatio="none" class="arrow-svg">
						<!-- 水平箭头 -->
						<template v-if="getArrowDirection(idx) !== 'vertical'">
							<!-- 向右箭头 -->
							<template v-if="getArrowDirection(idx) === 'right'">
								<line x1="10" y1="50" x2="60" y2="50"
									:stroke="isArrowActive(idx) ? '#33845b' : '#78848E'" stroke-width="36" />
								<polygon points="60,0 100,50 60,100"
									:fill="isArrowActive(idx) ? '#33845b' : '#78848E'" />
							</template>
							<!-- 向左箭头 -->
							<template v-else>
								<line x1="90" y1="50" x2="40" y2="50"
									:stroke="isArrowActive(idx) ? '#33845b' : '#78848E'" stroke-width="36" />
								<polygon points="40,0 0,50 40,100" :fill="isArrowActive(idx) ? '#33845b' : '#78848E'" />
							</template>
						</template>

						<!-- 垂直箭头 -->
						<template v-else>
							<line x1="50" y1="10" x2="50" y2="60" :stroke="isArrowActive(idx) ? '#33845b' : '#78848E'"
								stroke-width="20" />
							<polygon points="20,60 50,100 80,60" :fill="isArrowActive(idx) ? '#33845b' : '#78848E'" />
						</template>
					</svg>
				</div>
			</template>
...
<script setup>
	...
	//import { ref, reactive, defineEmits, defineExpose, defineProps, watch, nextTick } from "vue";
	const {
		proxy
	} = getCurrentInstance();
	...
	const showNodeDetail = (node) => {
		if(node.Children.length>0){
			proxy.$parent.showNodeDetail(node)
		}		
	}
