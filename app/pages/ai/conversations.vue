<template>
  <view class="conversation-page">
    <view class="top-bar">
      ...
      <view class="title">{{ displayAppName }}</view>
    ...
    <view class="app-panel">
      ...
      <view class="app-info">
        <view class="app-name">{{ displayAppName }}</view>
        <view class="app-desc">已授权应用，可继续历史会话或发起新的对话。</view>
      </view>
    </view>
...
const openNewChat = () => {
  uni.navigateTo({
    url: `/pages/ai/chat?appId=${appId.value}&appName=${encodeURIComponent(displayAppName.value || '')}`
  })
}

const openChat = (item) => {
  uni.navigateTo({
    url: `/pages/ai/chat?appId=${appId.value}&appName=${encodeURIComponent(displayAppName.value || '')}&conversationId=${encodeURIComponent(item.id)}&conversationName=${encodeURIComponent(item.name || '')}`
  })
}
