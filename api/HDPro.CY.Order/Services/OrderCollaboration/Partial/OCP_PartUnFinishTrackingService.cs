/*
 *所有关于OCP_PartUnFinishTracking类的业务代码应在此处编写
*可使用repository.调用常用方法，获取EF/Dapper等信息
*如果需要事务请使用repository.DbContextBeginTransaction
*也可使用DBServerProvider.手动获取数据库相关信息
*用户信息、权限、角色等使用UserContext.Current操作
*OCP_PartUnFinishTrackingService对增、删、改查、导入、导出、审核业务代码扩展参照ServiceFunFilter
*/
...
{
    public partial class OCP_PartUnFinishTrackingService
    {
        private readonly IOCP_PartUnFinishTrackingRepository _repository;//访问数据库
        private readonly IOCP_AlertRulesRepository _alertRulesRepository;//预警规则Repository
        private readonly ILogger<OCP_PartUnFinishTrackingService> _logger;//日志记录器

        [ActivatorUtilitiesConstructor]
        public OCP_PartUnFinishTrackingService(
            IOCP_PartUnFinishTrackingRepository dbRepository,
            IHttpContextAccessor httpContextAccessor,
            IOCP_AlertRulesRepository alertRulesRepository,
            ILogger<OCP_PartUnFinishTrackingService> logger
            )
        ...
        /// 可在此处添加OCP_PartUnFinishTracking特有的初始化代码
        /// </summary>
        protected override void InitCYOrderSpecific()
        {
            base.InitCYOrderSpecific();
            // 在此处添加OCP_PartUnFinishTracking特有的初始化逻辑
        }

        /// <summary>
        /// 重写CY.Order项目通用数据验证方法
        /// 可在此处添加OCP_PartUnFinishTracking特有的数据验证逻辑
        /// </summary>
        /// <param name="entity">要验证的实体</param>
        /// <returns>验证结果</returns>
        protected override WebResponseContent ValidateCYOrderEntity(OCP_PartUnFinishTracking entity)
        {
            var response = base.ValidateCYOrderEntity(entity);
            
            // 在此处添加OCP_PartUnFinishTracking特有的数据验证逻辑
        ...
        /// </summary>
        public override PageGridData<OCP_PartUnFinishTracking> GetPageData(PageDataOptions options)
        {
            ...
            SummaryExpress = (IQueryable<OCP_PartUnFinishTracking> queryable) =>
            {
            ...
            GetPageDataOnExecuted = (PageGridData<OCP_PartUnFinishTracking> grid) =>
            {
                // 可对查询的结果的数据操作
                List<OCP_PartUnFinishTracking> trackingRecords = grid.rows;
        ...
        /// 计算超期天数
        /// </summary>
        /// <param name="record">部件跟踪记录</param>
        /// <returns>超期天数</returns>
        private int CalculateOverdueDays(OCP_PartUnFinishTracking record)
        {
            try
            {
                // 如果没有计划完工日期，无法计算超期天数
        ...
        private void ApplyAlertWarningToData(List<OCP_PartUnFinishTracking> list)
        {
            try
            {
                var rules = _alertRulesRepository.FindAsync(x =>
                    x.AlertPage == "OCP_PartUnFinishTracking" &&
