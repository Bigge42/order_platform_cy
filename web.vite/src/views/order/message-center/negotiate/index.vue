...
import extend from '@/extension/order/message-center/negotiate/index.jsx'
...
const handleReplySubmit = async (formData) => {
  try {
    ...
    // 调用协商回复API接口
    const response = await http.post('api/OCP_NegotiationReply/add', {
      mainData: {
        ...formData,
        // 确保字段名正确映射
        NegotiationDate: formData.NegotiationDate,
    ...
    if (response && response.status === true) {
      ...
      // 刷新表格数据
      if (gridRef.value && gridRef.value.search) {
        await gridRef.value.search()
      }
