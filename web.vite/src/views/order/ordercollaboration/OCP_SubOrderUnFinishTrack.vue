...
import extend from '@/extension/order/ordercollaboration/OCP_SubOrderUnFinishTrack.jsx'
import viewOptions from './OCP_SubOrderUnFinishTrack/options.js'
...
const onInit = async ($vm) => {
  ...
  // 设置快捷查询字段
  gridRef.queryFields = ['MtoNo', 'MaterialNumber', 'Urgency', 'DeliveryNo']
      ...
      return (
        <div>
          <el-button
            ...
            onClick={($e) => handleJumpToShortageWO(row)}
          >
            跳转缺料
          </el-button>
          {/* <el-button 
...
const handleFollowUpSend = async (reminderData) => {
  ...
  try {
    ...
    const submitData = {
      ...
      // 委外负责人姓名（指定负责人）- 取选中人的userTrueName
...
const handleJumpToShortageWO = (row) => {
  ...
  const { MaterialNumber, POBillNo } = row
  ...
  proxy.$tabs.open({
    ...
    query: {
      ...
      BillNo: encodeURIComponent(POBillNo || '')
    }
  })
...
const handleJumpToSubOrder = (row) => {
  ...
  proxy.$tabs.open({
    ...
    query: {
      MaterialNumber: encodeURIComponent(MaterialNumber || ''),
      PurchaseBillNo: encodeURIComponent(POBillNo || '')
    }
  })
