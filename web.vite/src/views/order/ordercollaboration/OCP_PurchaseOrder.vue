    ...
    <view-grid ref="grid" :columns="columns" :detail="detail" :details="details" :editFormFields="editFormFields"
        ...
        <template #gridFooter>
            <div class="purchase-order-detail">
                <purchase-order-detail ref="purchaseOrderDetailRef"></purchase-order-detail>
            </div>
        </template>
    </view-grid>
...
//引用生成的vue文件(明细表)
import purchaseOrderDetail from './OCP_PurchaseOrderDetail.vue'
import extend from "@/extension/order/ordercollaboration/OCP_PurchaseOrder.jsx";
import viewOptions from './OCP_PurchaseOrder/options.js'
...
const purchaseOrderDetailRef = ref();
...
const onInit = async ($vm) => {
    ...
    // 设置快捷查询字段
    gridRef.queryFields = ['BillNo', 'SupplierName', 'Urgency'];
    
    //与jsx中的this.xx使用一样，只需将this.xx改为gridRef.xx

    //缓存主表方法，返回主表选中的行，在下面的明细表中会调用
    proxy.base.setItem('getPurchaseOrderSelectRow', () => {
...
const searchAfter = async (rows, result) => {
    // 主表查询完成后，清空明细表数据
    if (purchaseOrderDetailRef.value && purchaseOrderDetailRef.value.$refs.grid) {
        const detailTable = purchaseOrderDetailRef.value.$refs.grid.getTable(true)
...
const rowClick = ({ row, column, event }) => {
    ...
    //加载明细表
    purchaseOrderDetailRef.value.$refs.grid.search();
}
...
const handleUrge = (row) => {
    ...
    const reminderData = {
        ...
        id: row.Id || row.id,
        contractNo: row.PurchaseOrderCode || row.purchaseOrderCode || row.OrderCode || row.orderCode, // 使用采购订单编码
        planTrackNo: `PO${(row.Id || row.id || '').toString().slice(-6)}`, // 生成采购订单跟踪号
        business: 'purchase', // 业务类型为采购
