 ...
 *业务请在@/extension/order/ordercollaboration/OCP_PartPrdMODetail.jsx或OCP_PartPrdMODetail.vue文件编写
...
import extend from "@/extension/order/ordercollaboration/OCP_PartPrdMODetail.jsx";
import viewOptions from './OCP_PartPrdMODetail/options.js'
            ...
            return (
                <div>
                    ...
                    <el-button 
                        ...
                        link 
                        style={{padding: '2px 4px', fontSize: '12px'}} 
                        onClick={($e) => handleJumpToShortageBJ(row)}>跳转缺料</el-button>
                </div>
            )
        }
    })
...
const searchBefore = async (param) => {
    ...
    //返回false，则不会执行查询
    const partPrdMORows = getPartPrdMOSelectRow();
    if (!partPrdMORows.length) {
        proxy.$message.error('请先选择【配件生产制造订单】数据')
        return false;
    }
    //查询前获取配件生产制造订单的id，查询明细表
    param.wheres.push({ name: "FID", value: partPrdMORows[0].FID })
    return true;
}
...
const addBefore = async (formData) => {
    //明细表新建前获取【配件生产制造订单】选中的主键FID值(若不获取，写入的明细表数据无法知道属于哪个配件生产订单)
    const FID = getPartPrdMOSelectRow()[0].FID;
    formData.mainData['FID'] = FID
    return true;
}
...
//获取OCP_PartPrdMO.vue中缓存的方法，读取配件生产制造订单主表选中的行，保存数据时可以知道是哪条配件生产订单数据的明细
const getPartPrdMOSelectRow = () => {
    const fn = proxy.base.getItem('getPartPrdMOSelectRow');
    return fn();
}
...
const handleUrge = (row) => {
    ...
    const mainOrderInfo = partPrdMORows[0];
...
const handleReminderSend = async (reminderData) => {
    ...
    try {
        ...
        if (!mainOrder) {
            ...
            ElMessage.error('获取主表数据失败，请重新选择配件生产制造订单')
            return;
        }
        ...
        const submitData = {
            ...
            BillNo: mainOrder.ProductionOrderNo || mainOrder.BillNo || '',
            // 默认负责人姓名 - 取默认负责人的name
            ...
            // 配件负责人姓名（指定负责人）- 取选中人的userTrueName
...
const handleNegotiate = (row) => {
    ...
    const negotiationData = {
        ...
        BusinessType: "BJ",
        BusinessKey: row.FENTRYID,
        BillNo: mainOrderInfo.ProductionOrderNo || mainOrderInfo.BillNo || '',
...
const handleOpenMessageBoard = (row) => {
    ...
    // 传入业务类型和业务键值
    messageBoardRef.value?.open("BJ", row.FENTRYID)
}
...
const handleJumpToTrackBJ = (row) => {
    ...
    const partPrdMORows = getPartPrdMOSelectRow();
    if (!partPrdMORows.length) {
        proxy.$message.error('请先选择【部件生产制造订单】数据')
        return;
    }
    
    const mainOrderInfo = partPrdMORows[0];
    const { MaterialNumber } = row;
    const { ProductionOrderNo } = mainOrderInfo;
    ...
    proxy.$tabs.open({
        ...
        query: {
            ...
            ProductionOrderNo: encodeURIComponent(ProductionOrderNo || '')
        }
    });
...
// 跳转部件缺料结果
const handleJumpToShortageBJ = (row) => {
    console.log('跳转部件缺料结果:', row)
    ...
    proxy.$tabs.open({
        ...
        query: {
            ...
            BillNo: encodeURIComponent(ProductionOrderNo || '')
        }
    });

    proxy.$tabs.clearCache('OCP_LackMtrlResult_MO_BJ')
}
