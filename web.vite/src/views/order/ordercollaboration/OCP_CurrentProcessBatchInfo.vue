...
import extend from "@/extension/order/ordercollaboration/OCP_CurrentProcessBatchInfo.jsx";
import viewOptions from './OCP_CurrentProcessBatchInfo/options.js'
...
const searchBefore = async (param) => {
    ...
    //返回false，则不会执行查询
    let SaleOrderEntryData = getSaleOrderEntryData();
    console.log(SaleOrderEntryData);
    if (SaleOrderEntryData == null) {
        proxy.$message.error('请先选择【销售订单明细】数据')
        return false;
    }
    //查询前获取采购单的id，查询明细表
    param.wheres.push({ name: "MtoNo", value: SaleOrderEntryData.value.MtoNo })
    return true;
}
...
const getSaleOrderEntryData = () => {
    const fn = proxy.base.getItem('SaleOrderEntryData');
    return fn();
}
//监听表单输入，做实时计算
