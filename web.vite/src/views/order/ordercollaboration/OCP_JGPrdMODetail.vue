 ...
 *业务请在@/extension/order/ordercollaboration/OCP_JGPrdMODetail.jsx或OCP_JGPrdMODetail.vue文件编写
...
import extend from "@/extension/order/ordercollaboration/OCP_JGPrdMODetail.jsx";
import viewOptions from './OCP_JGPrdMODetail/options.js'
            ...
            return (
                <div>
                    ...
                    <el-button 
                        ...
                        link 
                        style={{padding: '2px 4px', fontSize: '12px'}} 
                        onClick={($e) => handleJumpToShortageJG(row)}>跳转缺料</el-button>
                </div>
            )
        }
    })
...
const searchBefore = async (param) => {
    ...
    //返回false，则不会执行查询
    const jgPrdMORows = getJGPrdMOSelectRow();
    if (!jgPrdMORows.length) {
        proxy.$message.error('请先选择【生产制造订单】数据')
        return false;
    }
    //查询前获取生产制造订单的id，查询明细表
    param.wheres.push({ name: "FID", value: jgPrdMORows[0].FID })
    return true;
}
...
const addBefore = async (formData) => {
    //明细表新建前获取【生产制造订单】选中的主键FID值(若不获取，写入的明细表数据无法知道属于哪个生产订单)
    const FID = getJGPrdMOSelectRow()[0].FID;
...
//获取OCP_JGPrdMO.vue中缓存的方法，读取生产制造订单主表选中的行，保存数据时可以知道是哪条生产订单数据的明细
const getJGPrdMOSelectRow = () => {
    const fn = proxy.base.getItem('getJGPrdMOSelectRow');
    return fn();
}
...
const handleUrge = (row) => {
    ...
    const mainOrderInfo = jgPrdMORows[0];
...
const handleReminderSend = async (reminderData) => {
    ...
    try {
        ...
        if (!mainOrder) {
            ...
            ElMessage.error('获取主表数据失败，请重新选择生产制造订单')
            return;
        }
        ...
        const submitData = {
            ...
            BillNo: mainOrder.ProductionOrderNo || '',
            // 默认负责人姓名 - 取默认负责人的name
            ...
            // 金工负责人姓名（指定负责人）- 取选中人的userTrueName
...
const handleNegotiate = (row) => {
    ...
    negotiationDialogData.value = {
        BusinessType: "JG",
        BusinessKey: row.FENTRYID,
        BillNo: mainOrderInfo.ProductionOrderNo,
        SupplierCode: mainOrderInfo.SupplierCode || '',
        // 保存当前行信息和主表信息，用于提交时使用
        currentRowData: row,
        mainOrderData: mainOrderInfo
...
const handleOpenMessageBoard = (row) => {
    ...
    // 传入业务类型和业务键值
    messageBoardRef.value?.open("JG", row.FENTRYID)
}
...
const handleJumpToTrackJG = (row) => {
    ...
    proxy.$tabs.open({
        ...
        query: {
            ...
            MOBillNo: encodeURIComponent(ProductionOrderNo || '')
        }
    });
...
// 跳转金工缺料
const handleJumpToShortageJG = (row) => {
    console.log('跳转金工缺料:', row)
