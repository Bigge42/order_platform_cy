...
<template>
    <view-grid ref="grid" :columns="columns" :detail="detail" :details="details" :editFormFields="editFormFields"
        ...
        <template #gridFooter>
            <div class="prd-mo-detail">
                <PrdMODetail ref="prdMODetailRef"></PrdMODetail>
            </div>
        </template>
    </view-grid>
...
//引用生成的vue文件(明细表)
import PrdMODetail from './OCP_PrdMODetail.vue'
import extend from "@/extension/order/ordercollaboration/OCP_PrdMO.jsx";
import viewOptions from './OCP_PrdMO/options.js'
...
const onInit = async ($vm) => {
    ...
    // 设置快捷查询字段
    gridRef.queryFields = ['ProductionOrderNo', 'Urgency', 'ProductionType'];
    
    //与jsx中的this.xx使用一样，只需将this.xx改为gridRef.xx

    //缓存主表方法，返回主表选中的行，在下面的明细表中会调用
    proxy.base.setItem('getPrdMOSelectRow', () => {
...
const searchAfter = async (rows, result) => {
    // 主表查询完成后，清空明细表数据
    if (prdMODetailRef.value && prdMODetailRef.value.$refs.grid) {
        const detailTable = prdMODetailRef.value.$refs.grid.getTable(true)
...
const rowClick = ({ row, column, event }) => {
    ...
    //加载明细表
    prdMODetailRef.value.$refs.grid.search();
}
...
const handleUrge = (row) => {
    ...
    const reminderData = {
        ...
        BusinessKey: row.FID,
        id: row.Id || row.id,
        contractNo: row.MOCode || row.moCode || row.OrderCode || row.orderCode, // 使用整机生产订单编码
        planTrackNo: `ZM${(row.Id || row.id || '').toString().slice(-6)}`, // 生成整机生产订单跟踪号
        business: 'machine_manufacturing_order', // 业务类型为整机生产订单
        defaultResponsible: '整机车间', // 默认负责人
