 ...
 *业务请在@/extension/order/ordercollaboration/OCP_JGUnFinishTrack.jsx或OCP_JGUnFinishTrack.vue文件编写
...
import extend from '@/extension/order/ordercollaboration/OCP_JGUnFinishTrack.jsx'
import viewOptions from './OCP_JGUnFinishTrack/options.js'
...
const onInit = async ($vm) => {
  ...
  // 设置快捷查询字段
  gridRef.queryFields = ['PlanTraceNo', 'MaterialNumber', 'Urgency', 'CurrentProcess']
      ...
      return (
        <div>
          <el-button
            ...
            onClick={($e) => handleJumpToShortageJG(row)}
          >
            跳转缺料
          </el-button>
          {/* <el-button 
...
const handleUrge = (row) => {
  ...
  const reminderData = {
    ...
    DefaultResPersonLoginName: row.DefaultResPersonLoginName || row.DefaultResPerson || '',
    // 保存当前行信息，用于提交时使用
    currentRowData: row
  }
...
const handleOpenMessageBoard = (row) => {
  ...
  // 传入业务类型和业务键值
  messageBoardRef.value?.open('JG', row.TrackID)
}

// 跳转金工缺料
const handleJumpToShortageJG = (row) => {
  console.log('跳转金工缺料:', row)
  const { MaterialNumber, ProductionOrderNo, BillNo } = row
  ...
  proxy.$tabs.open({
    ...
    query: {
      ...
      BillNo: encodeURIComponent(ProductionOrderNo || BillNo || '')
    }
  })
...
const handleJumpToJGPrdMO = (row) => {
  ...
  proxy.$tabs.open({
    ...
    query: {
      MaterialNumber: encodeURIComponent(MaterialNumber || ''),
      ProductionOrderNo: encodeURIComponent(ProductionOrderNo || BillNo || '')
    }
  })
