 ...
 *业务请在@/extension/order/ordercollaboration/OCP_PrdMODetail.jsx或OCP_PrdMODetail.vue文件编写
...
import extend from "@/extension/order/ordercollaboration/OCP_PrdMODetail.jsx";
import viewOptions from './OCP_PrdMODetail/options.js'
...
const searchBefore = async (param) => {
    ...
    //返回false，则不会执行查询
    const prdMORows = getPrdMOSelectRow();
    if (!prdMORows.length) {
        proxy.$message.error('请先选择【生产制造订单】数据')
        return false;
    }
    //查询前获取生产制造订单的id，查询明细表
    param.wheres.push({ name: "FID", value: prdMORows[0].FID })
    return true;
}
...
const addBefore = async (formData) => {
    ...
    const FID = getPrdMOSelectRow()[0].FID;
...
//获取OCP_PrdMO.vue中缓存的方法，读取生产制造订单主表选中的行，保存数据时可以知道是哪条生产订单数据的明细
const getPrdMOSelectRow = () => {
    const fn = proxy.base.getItem('getPrdMOSelectRow');
    return fn();
}
...
const handleUrge = (row) => {
    ...
    const mainOrderInfo = prdMORows[0];
...
const handleOpenMessageBoard = (row) => {
    ...
    // 传入业务类型和业务键值
    messageBoardRef.value?.open("ZP", row.FENTRYID)
}
