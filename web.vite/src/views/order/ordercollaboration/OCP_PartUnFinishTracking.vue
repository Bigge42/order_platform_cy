 ...
 *业务请在@/extension/order/ordercollaboration/OCP_PartUnFinishTracking.jsx或OCP_PartUnFinishTracking.vue文件编写
...
import extend from '@/extension/order/ordercollaboration/OCP_PartUnFinishTracking.jsx'
import viewOptions from './OCP_PartUnFinishTracking/options.js'
...
const onInit = async ($vm) => {
  ...
  // 设置快捷查询字段
  gridRef.queryFields = ['PlanTraceNo', 'ProductionOrderNo', 'MOBillNo', 'MESBatchNo']
      ...
      return (
        <div>
          <el-button
            ...
            onClick={($e) => handleJumpToShortageBJ(row)}
          >
            跳转缺料
          </el-button>
          {/* <el-button 
...
const handleOpenMessageBoard = (row) => {
  ...
  // 传入业务类型和业务键值
  messageBoardRef.value?.open('BJ', row.TrackID)
}
