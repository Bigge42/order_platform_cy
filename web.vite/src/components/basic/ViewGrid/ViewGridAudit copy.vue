<template>
    ...
    >
      <el-descriptions class="desc-top" :column="3" size="default" :border="true">
        ...
        >
          ...
          <div
            class="form-file"
            v-if="data.formType == 'file' || data.formType == 'excel'"
          >
            <a
          ...
          <div
            class="form-editor"
            v-else-if="data.formType == 'editor'"
            v-html="data.value"
          ></div>
          <div v-else>
            {{ data.value }}
  ...
  <audit-edit-form @saveEditForm="saveEditForm" :getTable="getTable" ref="editForm"></audit-edit-form>
  <!-- 加签 -->
  ...
  setup(props, { emit }) {
    ...
    const auditClick = () => {
      ...
      if (isAnti.value) {
        ...
        url = `api/${currentOption.table}/${mh}?auditReason=${
          auditParam.reason
        }&auditStatus=${auditParam.value < 0 ? 0 : auditParam.value}`;
        params = keys;
      }
    ...
    const saveEditForm = (formFields, callback) => {
      ...
      proxy.http
        .post(url, { mainData: formFields, isFlow: true }, "审核中....")
        .then((x) => {
          if (!x.status) {
            proxy.$message.error(x.message);
            return;
    ...
    const getTable=()=>{
      return props.option.url.replaceAll('/','')
    }
